# tictactoe_lab4b.py
# by Jerome Hayes Jr, Feb 7, 2017
#this is my attempted at solving the assignment. I couldn't figure it out.
...
class T3():
    ...
    # apply if player agrees;
    
    def put_X(self):
        self.present()
        while(True):
            pick = raw_input('Choose place for X: ')
            recommended = self.minimax()
            if pick!= recommended:
                print "The recomended movve is", recommended
            else:
                if self.ttt[pick] == 0:
                    self.ttt[pick] = 'X'
                    break
